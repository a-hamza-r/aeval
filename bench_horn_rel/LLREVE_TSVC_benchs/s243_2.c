#include "declarations.h"

//  node splitting
//  false dependence cycle breaking

int s243(int count) {
if (count <= 0 || count > 10) return 1;

  f[0] = a[0+1];
  a[0] = b[0] + c[0  ] * d[0];
  b[0] = a[0] + d[0  ] * e[0];
  a[0] = b[0] + f[0] * d[0];

  f[0+1] = a[0+1+1];
  a[0+1] = b[0+1] + c[0+1  ] * d[0+1];
  b[0+1] = a[0+1] + d[0+1  ] * e[0+1];
  a[0+1] = b[0+1] + f[0+1] * d[0+1];

  f[0+2] = a[0+2+1];
  a[0+2] = b[0+2] + c[0+2  ] * d[0+2];
  b[0+2] = a[0+2] + d[0+2  ] * e[0+2];
  a[0+2] = b[0+2] + f[0+2] * d[0+2];

  f[0+3] = a[0+3+1];
  a[0+3] = b[0+3] + c[0+3  ] * d[0+3];
  b[0+3] = a[0+3] + d[0+3  ] * e[0+3];
  a[0+3] = b[0+3] + f[0+3] * d[0+3];

  f[0+4] = a[0+4+1];
  a[0+4] = b[0+4] + c[0+4  ] * d[0+4];
  b[0+4] = a[0+4] + d[0+4  ] * e[0+4];
  a[0+4] = b[0+4] + f[0+4] * d[0+4];

  f[0+5] = a[0+5+1];
  a[0+5] = b[0+5] + c[0+5  ] * d[0+5];
  b[0+5] = a[0+5] + d[0+5  ] * e[0+5];
  a[0+5] = b[0+5] + f[0+5] * d[0+5];

  f[0+6] = a[0+6+1];
  a[0+6] = b[0+6] + c[0+6  ] * d[0+6];
  b[0+6] = a[0+6] + d[0+6  ] * e[0+6];
  a[0+6] = b[0+6] + f[0+6] * d[0+6];

  for (int i = 7; i < count*8-1; i+=8) {
    f[i] = a[i+1];
    a[i] = b[i] + c[i  ] * d[i];
    b[i] = a[i] + d[i  ] * e[i];
    a[i] = b[i] + f[i] * d[i];

    f[i+1] = a[i+1+1];
    a[i+1] = b[i+1] + c[i+1  ] * d[i+1];
    b[i+1] = a[i+1] + d[i+1  ] * e[i+1];
    a[i+1] = b[i+1] + f[i+1] * d[i+1];

    f[i+2] = a[i+2+1];
    a[i+2] = b[i+2] + c[i+2  ] * d[i+2];
    b[i+2] = a[i+2] + d[i+2  ] * e[i+2];
    a[i+2] = b[i+2] + f[i+2] * d[i+2];

    f[i+3] = a[i+3+1];
    a[i+3] = b[i+3] + c[i+3  ] * d[i+3];
    b[i+3] = a[i+3] + d[i+3  ] * e[i+3];
    a[i+3] = b[i+3] + f[i+3] * d[i+3];

    f[i+4] = a[i+4+1];
    a[i+4] = b[i+4] + c[i+4  ] * d[i+4];
    b[i+4] = a[i+4] + d[i+4  ] * e[i+4];
    a[i+4] = b[i+4] + f[i+4] * d[i+4];

    f[i+5] = a[i+5+1];
    a[i+5] = b[i+5] + c[i+5  ] * d[i+5];
    b[i+5] = a[i+5] + d[i+5  ] * e[i+5];
    a[i+5] = b[i+5] + f[i+5] * d[i+5];

    f[i+6] = a[i+6+1];
    a[i+6] = b[i+6] + c[i+6  ] * d[i+6];
    b[i+6] = a[i+6] + d[i+6  ] * e[i+6];
    a[i+6] = b[i+6] + f[i+6] * d[i+6];

    f[i+7] = a[i+7+1];
    a[i+7] = b[i+7] + c[i+7  ] * d[i+7];
    b[i+7] = a[i+7] + d[i+7  ] * e[i+7];
    a[i+7] = b[i+7] + f[i+7] * d[i+7];
  }
  return 0;
}

