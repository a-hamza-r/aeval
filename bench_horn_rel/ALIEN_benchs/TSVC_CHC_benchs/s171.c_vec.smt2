...
(rule (=> 
	(and 
		...
		(= a_array1 (store a_array (* i inc) a_i))

		(= a_i1 (+ (select a_array1 (* (+ i 1) inc)) (select b_array (+ i 1))))
		(= a_array2 (store a_array1 (* (+ i 1) inc) a_i1))

		(= a_i2 (+ (select a_array2 (* (+ i 2) inc)) (select b_array (+ i 2))))
		(= a_array3 (store a_array2 (* (+ i 2) inc) a_i2))

		(= a_i3 (+ (select a_array3 (* (+ i 3) inc)) (select b_array (+ i 3))))
		(= a_array4 (store a_array3 (* (+ i 3) inc) a_i3))

		(= a_i4 (+ (select a_array4 (* (+ i 4) inc)) (select b_array (+ i 4))))
		(= a_array5 (store a_array4 (* (+ i 4) inc) a_i4))

		(= a_i5 (+ (select a_array5 (* (+ i 5) inc)) (select b_array (+ i 5))))
		(= a_array6 (store a_array5 (* (+ i 5) inc) a_i5))

		(= a_i6 (+ (select a_array6 (* (+ i 6) inc)) (select b_array (+ i 6))))
		(= a_array7 (store a_array6 (* (+ i 6) inc) a_i6))

		(= a_i7 (+ (select a_array7 (* (+ i 7) inc)) (select b_array (+ i 7))))
		(= a_array8 (store a_array7 (* (+ i 7) inc) a_i7))
	)
	(loop a_array8 b_array (+ i 8) count inc)
))
(rule (=> 
	(and 
		(loop a_array b_array i count inc)
