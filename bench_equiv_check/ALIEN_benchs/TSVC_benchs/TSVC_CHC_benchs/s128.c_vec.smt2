...
(rule (=> 
	(and 
		...
		(= b_array1 (store b_array (* 2 i) (+ a_i c_k)))

		(= b_k1 (select b_array1 (* 2 (+ i 1))))
		(= c_k1 (select c_array (* 2 (+ i 1))))
		(= d_i1 (select d_array (+ i 1)))
		(= a_i1 (- b_k1 d_i1))
		(= a_array2 (store a_array1 (+ i 1) a_i1))
		(= b_array2 (store b_array1 (* 2 (+ i 1)) (+ a_i1 c_k1)))

		(= b_k2 (select b_array2 (* 2 (+ i 2))))
		(= c_k2 (select c_array (* 2 (+ i 2))))
		(= d_i2 (select d_array (+ i 2)))
		(= a_i2 (- b_k2 d_i2))
		(= a_array3 (store a_array2 (+ i 2) a_i2))
		(= b_array3 (store b_array2 (* 2 (+ i 2)) (+ a_i2 c_k2)))

		(= b_k3 (select b_array3 (* 2 (+ i 3))))
		(= c_k3 (select c_array (* 2 (+ i 3))))
		(= d_i3 (select d_array (+ i 3)))
		(= a_i3 (- b_k3 d_i3))
		(= a_array4 (store a_array3 (+ i 3) a_i3))
		(= b_array4 (store b_array3 (* 2 (+ i 3)) (+ a_i3 c_k3)))
	)
	(loop a_array4 b_array4 c_array d_array (+ i 4) count)
))
(rule (=> 
	(and 
		(loop a_array b_array c_array d_array i count)
