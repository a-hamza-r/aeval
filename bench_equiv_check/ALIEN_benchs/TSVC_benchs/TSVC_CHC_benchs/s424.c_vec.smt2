...
(rule (=> 
	(and 
		...
		(= array_i1 (+ (select array1 (+ i 1)) (select a_array (+ i 1))))
		(= array2 (store array1 (+ i 65) array_i1))

		(= array_i2 (+ (select array2 (+ i 2)) (select a_array (+ i 2))))
		(= array3 (store array2 (+ i 66) array_i2))

		(= array_i3 (+ (select array3 (+ i 3)) (select a_array (+ i 3))))
		(= array4 (store array3 (+ i 67) array_i3))

		(= array_i4 (+ (select array4 (+ i 4)) (select a_array (+ i 4))))
		(= array5 (store array4 (+ i 68) array_i4))

		(= array_i5 (+ (select array5 (+ i 5)) (select a_array (+ i 5))))
		(= array6 (store array5 (+ i 69) array_i5))

		(= array_i6 (+ (select array6 (+ i 6)) (select a_array (+ i 6))))
		(= array7 (store array6 (+ i 70) array_i6))
	)
...
(rule (=> 
	(and 
		...
		(= array_i7 (+ (select array7 (+ i 7)) (select a_array (+ i 7))))
		(= array8 (store array7 (+ i 71) array_i7))
	)
