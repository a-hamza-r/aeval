...
(rule (=> 
	(and 
		...
		(= a_i1 (+ (select a_array1 (+ (+ i 1) 1)) (select b_array (+ i 1))))
		(= a_array2 (store a_array1 (+ i 1) a_i1))

		(= a_i2 (+ (select a_array2 (+ (+ i 2) 1)) (select b_array (+ i 2))))
		(= a_array3 (store a_array2 (+ i 2) a_i2))

		(= a_i3 (+ (select a_array3 (+ (+ i 3) 1)) (select b_array (+ i 3))))
		(= a_array4 (store a_array3 (+ i 3) a_i3))

		(= a_i4 (+ (select a_array4 (+ (+ i 4) 1)) (select b_array (+ i 4))))
		(= a_array5 (store a_array4 (+ i 4) a_i4))

		(= a_i5 (+ (select a_array5 (+ (+ i 5) 1)) (select b_array (+ i 5))))
		(= a_array6 (store a_array5 (+ i 5) a_i5))

		(= a_i6 (+ (select a_array6 (+ (+ i 6) 1)) (select b_array (+ i 6))))
...
(rule (=> 
	(and 
		...
		(= a_i7 (+ (select a_array7 (+ (+ i 7) 1)) (select b_array (+ i 7))))
