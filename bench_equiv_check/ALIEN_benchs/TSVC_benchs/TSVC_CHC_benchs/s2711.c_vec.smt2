...
(rule (=> 
	(and 
		...
		(= a_i (+ (select a_array i) (* (select b_array i) (select c_array i))))
		(= a_array1 (store a_array i a_i))

		(= a_i1 (+ (select a_array (+ i 1)) (* (select b_array (+ i 1)) (select c_array (+ i 1)))))
		(= a_array2 (store a_array1 (+ i 1) a_i1))

		(= a_i2 (+ (select a_array (+ i 2)) (* (select b_array (+ i 2)) (select c_array (+ i 2)))))
		(= a_array3 (store a_array2 (+ i 2) a_i2))

		(= a_i3 (+ (select a_array (+ i 3)) (* (select b_array (+ i 3)) (select c_array (+ i 3)))))
		(= a_array4 (store a_array3 (+ i 3) a_i3))

		(= a_i4 (+ (select a_array (+ i 4)) (* (select b_array (+ i 4)) (select c_array (+ i 4)))))
		(= a_array5 (store a_array4 (+ i 4) a_i4))

		(= a_i5 (+ (select a_array (+ i 5)) (* (select b_array (+ i 5)) (select c_array (+ i 5)))))
		(= a_array6 (store a_array5 (+ i 5) a_i5))

		(= a_i6 (+ (select a_array (+ i 6)) (* (select b_array (+ i 6)) (select c_array (+ i 6)))))
		(= a_array7 (store a_array6 (+ i 6) a_i6))

		(= a_i7 (+ (select a_array (+ i 7)) (* (select b_array (+ i 7)) (select c_array (+ i 7)))))
