#include "declarations.h"

//  control flow
//  deriving assertions

int s162(int count, int k) {
if (count <= 0 || count > 10) return 1;
  if (k > 0) {
    a[0] = a[0 + k] + b[0] * c[0];
    a[0+1] = a[0+1 + k] + b[0+1] * c[0+1];
    a[0+2] = a[0+2 + k] + b[0+2] * c[0+2];
    a[0+3] = a[0+3 + k] + b[0+3] * c[0+3];
    a[0+4] = a[0+4 + k] + b[0+4] * c[0+4];
    a[0+5] = a[0+5 + k] + b[0+5] * c[0+5];
    a[0+6] = a[0+6 + k] + b[0+6] * c[0+6];
    for (int i = 7; i < count*8-1; i+=8) {
      a[i] = a[i + k] + b[i] * c[i];
      a[i+1] = a[i+1 + k] + b[i+1] * c[i+1];
      a[i+2] = a[i+2 + k] + b[i+2] * c[i+2];
      a[i+3] = a[i+3 + k] + b[i+3] * c[i+3];
      a[i+4] = a[i+4 + k] + b[i+4] * c[i+4];
      a[i+5] = a[i+5 + k] + b[i+5] * c[i+5];
      a[i+6] = a[i+6 + k] + b[i+6] * c[i+6];
      a[i+7] = a[i+7 + k] + b[i+7] * c[i+7];
    }
  }
  return 0;
}

